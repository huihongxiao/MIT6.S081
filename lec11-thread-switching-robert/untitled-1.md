...
yield函数只做了几件事情，它首先获取了进程的锁。实际上，在锁释放之前，进程的状态会变得不一致，例如，yield将要将进程的状态改为RUNABLE，表明进程并没有在运行，但是实际上这个进程还在运行，代码正在当前进程的内核线程中执行。所以这加锁的目的之一就是，即使我们将进程的状态改为了RUNABLE，其他的CPU核的调度器线程能看到进程的状态为RUNABLE并尝试运行它，因为进程现在加锁了。如果发生了那就惨了，因为那样的话一个进程就在两个CPU核上运行了，而一个进程只有一个栈，这意味着两个CPU核在同一个栈上运行代码。

接下来yield函数中将进程的状态改为RUNABLE。这里的意思是，当前进程要出让CPU，并切换到调度器线程。当前进程的状态会是RUNABLE意味着它还会再次运行，因为毕竟现在是一个定时器中断打断了当前正在运行的进程。

之后yield函数中调用了位于proc.c文件中的sched函数。我们进入到sched函数中，

![](../.gitbook/assets/image%20%28494%29.png)

可以看出，sched函数基本没有干任何事情，只是做了一些sannity检查，如有有异常再panic。为什么会有这么多检查？因为这里的XV6代码已经有很多年的历史了，这些代码经历过各种各样的bug，相应的这里有各种各样的sannity检查和panic来避免可能的bug。这里我将跳过所有的检查，直接走到位于底部的switch函数。

调用switch会保存当前的内核线程寄存器到p-&gt;context中。switch函数的另一个参数c-&gt;context，c表示当前CPU的结构体。CPU结构体中的context保存了当前CPU核的调度器线程的寄存器。所以我们将先保存当前的内核线程的状态，恢复当前CPU核的调度器线程，并继续指向当前CPU核的调度器线程。
